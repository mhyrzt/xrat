...
- Need to check which architectures the base image targets and whether both
  xray and sing-box publish matching musl/glibc builds.
...
- Unit test: encode a known config, pass it via the flag, confirm xrat reads
  the expected values.
...
When launched from the app menu or search, xrat's window shows the xrat icon
in the DE taskbar/dock instead of the terminal emulator's icon (kitty,
alacritty, etc.).
...
2. **Wrapper script with `StartupWMClass`**: Launch from a script, set
   `StartupWMClass=xrat` in the desktop entry, and configure the terminal to
   set WM_CLASS to `xrat`. Fragile and terminal-dependent.
