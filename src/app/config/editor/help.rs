...
pub(super) fn for_path(path: &str) -> Option<SettingHelp> {
    Some(match path {
        ...
        "routing.domain_strategy" => help(
            "Controls when Xray/V2Ray routing resolves domain names to IP addresses. sing-box ignores this field.",
            "AsIs, IPIfNonMatch, or IPOnDemand.",
            "domain_strategy = \"IPIfNonMatch\"",
        ),
        "routing.direct.domain" => help(
            "Routes matching domains directly in managed sessions. Direct rules are evaluated before block rules.",
            "Xray/V2Ray domain rules. sing-box supports bare keywords plus full:, domain:, keyword:, and regexp: forms.",
            "domain = [\"domain:example.com\", \"full:api.example.com\"]",
        ),
        "routing.direct.ip" => help(
            "Routes matching IP addresses or networks directly in managed sessions.",
            "IP addresses or CIDRs. Xray-only forms such as geoip: and ext: must use Xray/V2Ray.",
            "ip = [\"192.168.0.0/16\"]",
        ),
        "routing.direct.geosite" => help(
            "Routes Xray/V2Ray geosite categories directly.",
            "Category names such as private. Not yet supported by sing-box runtimes.",
            "geosite = [\"private\"]",
        ),
        "routing.direct.geoip" => help(
            "Routes Xray/V2Ray GeoIP categories directly.",
            "Category names such as private. Not yet supported by sing-box runtimes.",
            "geoip = [\"private\"]",
        ),
        "routing.block.domain" => help(
            "Blocks matching domains in managed sessions unless an earlier direct rule also matches.",
            "Xray/V2Ray domain rules. sing-box supports bare keywords plus full:, domain:, keyword:, and regexp: forms.",
            "domain = [\"domain:ads.example\"]",
        ),
        "routing.block.ip" => help(
            "Blocks matching IP addresses or networks in managed sessions unless an earlier direct rule also matches.",
            "IP addresses or CIDRs. Xray-only forms such as geoip: and ext: must use Xray/V2Ray.",
            "ip = [\"203.0.113.0/24\"]",
        ),
        "routing.block.geosite" => help(
            "Blocks Xray/V2Ray geosite categories.",
            "Category names such as category-ads-all. Not yet supported by sing-box runtimes.",
            "geosite = [\"category-ads-all\"]",
        ),
        "routing.block.geoip" => help(
            "Blocks Xray/V2Ray GeoIP categories.",
