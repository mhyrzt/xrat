...
use crate::db::models::{ImportSource, ImportSummary, SourceKind};
...
pub async fn import_nodes(
    ...
    let subscription_id = insert_subscription(pool, source).await?;
    ...
    let total_configs = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM configs")
        .fetch_one(pool)
        .await?;
