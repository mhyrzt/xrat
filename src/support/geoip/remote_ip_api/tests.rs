...
async fn resolves_country_city_and_asn_from_stubbed_service() {
    let server = spawn_json_server(
        ...
        json!({
            ...
            "as": "AS60781 LeaseWeb-AS",
            "org": "LeaseWeb"
        })
        .to_string(),
    )
    .await;
    let lookup = RemoteIpApiLookup::new(server, Duration::from_secs(1)).unwrap();
    let ip: IpAddr = "8.8.8.8".parse().unwrap();

    assert_eq!(lookup.country(ip).await.unwrap().as_deref(), Some("NL"));
    assert_eq!(
        lookup.city(ip).await.unwrap().as_deref(),
        Some("Amsterdam/North Holland/NL")
    );
    assert_eq!(
...
async fn reports_http_status_errors() {
    let server = spawn_json_server(StatusCode::FORBIDDEN, "denied".to_string()).await;
    let lookup = RemoteIpApiLookup::new(server, Duration::from_secs(1)).unwrap();
    let ip: IpAddr = "8.8.8.8".parse().unwrap();

    match lookup.country(ip).await.expect_err("status should fail") {
        GeoIpError::Status { status, .. } => assert_eq!(status, 403),
